...
3. **Arquitectura Híbrida (`ICodeRunner`):**
   - *Ventajas:* Lo mejor de ambos mundos:
     - **Modo Cliente (Wasm / Web Workers):** Para lenguajes interpretados/scripting (Python con `Pyodide`, Python con `Pyodide`, JavaScript en Web Worker). Latencia < 50 ms, cero costo de servidor.
...
- Para **Python**: ejecución en cliente mediante `Pyodide` (WebAssembly de Python 3.12).
...
## 4. Consecuencias
- **Positivas:**
  - Costo de infraestructura prácticamente nulo para los cursos de Python en el 90% de los ejercicios interactivos.
